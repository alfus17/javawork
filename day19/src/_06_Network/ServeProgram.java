...
public class ServeProgram {
	/*
	 * 현재 구동중인 서버가 있으면 클라이언트는 그 서버로 요청을 보낼수 잇음 => 응답결과가 돌아옴
	 *  - 요청과 응답에 의해서 프로그램이 돌아감("통신" 한다라고 표현 )
	 *  - 웹에서는 통신방식 : http 프로토콜 통신(https)
	 
	 *	자바만을 가지고 서버와 클라이언트간에 간단한 통신하기
	 	이때 데이터를 입출력하고자 한다면 서버와 클라이언트간에 스트림이(연결통로)가 필요
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 3 . Client의 요청이 올 때까지 대기 상태
			System.out.println("클라이언트의 요청을 기다리고 있음...");
			
			// 4.  연결 오청이 들어오면 수락 후 해당 클라이언트와 통신할 수 있는 서버측 소켓 객체 생성
			Socket socket = server.accept();
			System.out.println(socket.getInetAddress().getHostAddress() + " 가 연결 요청함!");
			
			/*// 5. 클라이언트와 입출력 기반 스트림 생성(바이트 스트림 밖에 안됨)
